import { AlertCircle, ArrowUpRight, Building2, CheckSquare, ChevronLeft, ChevronRight, RefreshCw, Search, Trash2, UserRound, UsersRound } from 'lucide-react';
...
export default function Prestadores() {
  ...
  return <div className="space-y-7">
    ...
    {deleteMessage && <p className="rounded-lg bg-primary/10 px-3 py-2 text-sm text-primary">{deleteMessage}</p>}
     ...
     </div>)}</div><div className="mt-5 flex items-center justify-between rounded-xl border border-border bg-card px-3 py-2"><span className="text-xs text-muted-foreground">Página {page} de {pageCount} · {providers.length} prestador(es)</span><div className="flex gap-1"><button type="button" aria-label="Página anterior" onClick={() => setPage((current) => Math.max(1, current - 1))} disabled={page === 1} className="grid h-8 w-8 place-items-center rounded-lg border border-border disabled:opacity-40"><ChevronLeft className="h-4 w-4" /></button><button type="button" aria-label="Próxima página" onClick={() => setPage((current) => Math.min(pageCount, current + 1))} disabled={page === pageCount} className="grid h-8 w-8 place-items-center rounded-lg border border-border disabled:opacity-40"><ChevronRight className="h-4 w-4" /></button></div></div></> : <div className="rounded-2xl border border-dashed border-border bg-card p-14 text-center"><div className="mx-auto mb-4 grid h-12 w-12 place-items-center rounded-2xl bg-secondary text-muted-foreground"><UsersRound className="h-5 w-5" /></div><h3 className="font-bold">{search ? 'Nenhum prestador encontrado' : 'Diretório vazio'}</h3><p className="mx-auto mt-1 max-w-xs text-sm text-muted-foreground">{search ? 'Tente uma busca diferente.' : 'Cadastre o primeiro prestador para começar.'}</p>{!search && <Link href="/cadastro" data-testid="link-empty-cadastro" className="mt-5 inline-flex text-xs font-semibold text-primary hover:underline">Cadastrar agora</Link>}</div>}
  </div>;
}
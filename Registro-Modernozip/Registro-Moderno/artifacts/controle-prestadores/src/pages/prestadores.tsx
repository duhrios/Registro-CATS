import { ArrowUpRight, Building2, CheckSquare, Search, Trash2, UserRound, UsersRound } from 'lucide-react';
...
export default function Prestadores() {
  ...
  return <div className="space-y-7">
    ...
    {query.isLoading ? <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{[1,2,3,4,5,6].map((i) => <div key={i} className="h-40 animate-pulse rounded-2xl border border-border bg-card p-5"><div className="skeleton h-11 w-11 rounded-xl" /><div className="skeleton mt-4 h-3 w-2/3 rounded" /><div className="skeleton mt-2 h-2 w-1/2 rounded" /></div>)}</div> : query.isError ? <div className="rounded-2xl border border-border bg-card p-12 text-center"><p className="font-semibold">Diretório indisponível</p><p className="mt-1 text-sm text-muted-foreground">Não conseguimos carregar os prestadores agora.</p></div> : providers.length ? <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{providers.map((provider) => <div key={provider.id} data-testid={`card-provider-${provider.id}`} className="group rounded-2xl border border-border bg-card p-5 transition-all hover:-translate-y-0.5 hover:border-primary/35 hover:shadow-[0_10px_30px_hsl(174_43%_32%/.08)]">
